...
import { addMatrix, fetchMatrices, fetchMatrixHierarchy } from "../../services/matrix";
  ...
  const [openModal, setOpenModal] = useState(false);
    ...
    useEffect(() => {
      const loadMatrices = async () => {
        const data = await fetchMatrixHierarchy();
...

              {
                ...
                matrices.map((matrix) => (
                  <tr class="border-b border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <td class="px-6 py-4">{matrix.label}</td>{" "}
                    <td class="px-6 py-4">{matrix.generated_label || ""}</td>
                    <td class="px-6 py-4">{matrix.createdAt || ""}</td>
                    <td class="px-6 py-4 text-right">
                      <Link
                        class="font-medium text-blue-600 hover:underline dark:text-blue-500"
                        to={`/matrices/show/${matrix.id}`}
                      >
                        View
                      </Link>
                    </td>
...
export default MatrixHierarchy